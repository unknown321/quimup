...
qm_trayIcon::qm_trayIcon(QMainWindow *parent) : QSystemTrayIcon(parent) {
  ...
  QObject::connect(a_previous, SIGNAL(triggered()), this,
                   SLOT(on_prev_action()));
  ...
  QObject::connect(a_playpause, SIGNAL(triggered()), this,
                   SLOT(on_play_action()));
...
void qm_trayIcon::on_quit_action() { emit signal_tray_menu(ID_exit); }
