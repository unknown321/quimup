...
int main(int argc, char **argv) {
  ...
  for (int a = 1; a < argc; ++a) {
    ...
    if (msg == "-p" || msg == "-play") {
    ...
    } else if (msg == "-i" || msg == "-instance") {
      b_multiple_instances = true;
    } else if (msg == "-l" || msg == "-locale") {
      if (a < (argc - 1)) {
        locale = argv[a + 1];
        // possible stray '-l'
      ...
      printf("This program is licensed by the GPL and distributed in the hope "
             "that it will be useful, but without any warranty.\n");
      printf("----\ncommand line parameters:\n");
      printf(" -h(elp)           show this information and exit\n");
      printf(" -i(nstance)       force a new instance of Quimup\n");
      printf(" -l(ocale) xx      use locale 'xx' (fr, po, en_GB, etc) or "
             "'none' to force internal locale\n");
      printf(" -l(ocale) file    use locale from full path to quimup_xx.qm "
             "file\n");
      printf(
          " -p(lay) %%U        play files in %%U (or %%F) in new playlist\n");
      printf("  %%U               append files in %%U (or %%F) to the "
             "playlist\n----\n");
      return 0;
    } else // if URL
        if (msg.startsWith("file://")) {
      // we have a string in URL format, so convert special chars:
      msg = (QUrl::fromEncoded(static_cast<const char *>(msg.toUtf8())))
                .toString();
      message.append(msg);
      filecount++;
    } else //  if plain path
  ...
  if (locale != "none") {
    if (!b_locale_default) {
      ...
      if (locale.startsWith("/") &&
          locale.endsWith(".qm")) // full path to a file
      {
  ...
  QObject::connect(&app, SIGNAL(messageReceived(const QString &)), core,
                   SLOT(on_message_from_2nd_instance(const QString &)));
