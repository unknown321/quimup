...
bool qm_mpdCom::mpd_connect()
{
    ...
    conn = mpd_connection_new( static_cast<const char*>(server_name.toUtf8()), static_cast<uint>(server_port) ,30000);
    ...
    if (!server_password.isEmpty())
    {
        ...
        //{
            mpd_send_password(conn, static_cast<const char*>(server_password.toUtf8()));
            mpd_response_finish(conn);
            if ( mpd_connection_get_error(conn) != MPD_ERROR_SUCCESS )
            {
...
bool qm_mpdCom::error_check(QString action)
{
    if (conn == nullptr)
    {
        mpd_disconnect(false);
        emit sgnl_connected(false);
        //printf ("Error : on \"" + action.toUtf8().constData() + "\" : connection is NULL\n");
    ...
    if (mpd_connection_get_error(conn) != MPD_ERROR_SUCCESS)
    {
        ...
        if (mpd_connection_get_error(conn) != MPD_ERROR_CLOSED)
        {
            ...
            {
                QString msg  = "Non fatal error : on \"" + action + "\": " + mpd_connection_get_error_message(conn)  + "\n";
                printf ("%s", msg.toUtf8().constData());
            ...
            {
                QString msg  = "Connection error : on \"" + action + "\": " + mpd_connection_get_error_message(conn)  + "\n";
                printf ("%s", msg.toUtf8().constData());
        ...
        else
        {
            ...
            QString msg  = "Fatal error : on \"" + action + "\": " + mpd_connection_get_error_message(conn)  + "\n";
            printf ("%s", msg.toUtf8().constData());
        }
...
bool qm_mpdCom::status_check()
{
    ...
    if( (current_songID != mpd_status_get_song_id(servStatus))  || b_reload)
    {
        ...
        if (entity == nullptr || !error_check("status_check : mpd_send_current_song"))
        {
            current_songpos = -1;
            current_songID = -1;
            song = nullptr;
...
int qm_mpdCom::execute_cmds(qm_commandList cmdlist, bool resetlist)
{
    ...
    while (itr.hasNext())
    {
        ...
        switch (curCommand.cmd)
        {
            ...
            {
                if ( mpd_send_rm(conn, static_cast<const char*>(curCommand.file.toUtf8())))
                    results++;
                break;
            }
            ...
            {
                current_playlist++; // prevent list update
                if (mpd_send_add(conn, static_cast<const char*>(curCommand.file.toUtf8())))
                {
                    plist_lenght++;
                    results++;
                }
            ...
            case CMD_INS:
            {
                ...
                if (mpd_send_add(conn, static_cast<const char*>(curCommand.file.toUtf8())))
                {
                    ...
                    // move into position
                    current_playlist++; // prevent list update
                    mpd_send_move(conn, plist_lenght-1, static_cast<uint>(curCommand.moveTo));
                }
                break;
            }
            ...
            {
                current_playlist++; // prevent list update
                if (mpd_send_move_id(conn,  static_cast<uint>(curCommand.songid), static_cast<uint>(curCommand.moveTo)))
                    results++;
                break;
            }
            ...
            {
                current_playlist++; // prevent list update
                if (mpd_send_move(conn,  static_cast<uint>(curCommand.songnr), static_cast<uint>(curCommand.moveTo)))
                    results++;
                break;
            }
            ...
            case CMD_SCN:
            {
                b_dbase_updating = true;
                current_playlist++; // prevent list update
                if (mpd_send_command(conn, "rescan", static_cast<const char*>(curCommand.file.toUtf8()), nullptr))
                {
                    results++;
                }
                break;
            ...
            case CMD_UPD:
            {
                b_dbase_updating = true;
                current_playlist++; // prevent list update
                if (mpd_send_command(conn, "update", static_cast<const char*>(curCommand.file.toUtf8()), nullptr))
                {
                    results++;
                }
                break;
...
QString qm_mpdCom::save_selection(QStringList tobeSaved, QString listname)
{
    ...
    {
        mpd_send_playlist_add(conn, static_cast<const char*>(listname.toUtf8()), static_cast<const char*>(tobeSaved.at(i).toUtf8()));
    }
    mpd_command_list_end(conn);
    mpd_response_finish(conn);
...
/*
    ...
    for (int i = 0; i < albumlist.size(); ++i)
    {
        ...
        if ( mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM, static_cast<const char*>(albumlist.at(i).toUtf8()))
...
/* // DEBUG >>
    ...
    for (int i = 0; i < artistlist.size(); ++i)
    {
        ...
        const char* searchfor = static_cast<const char*>(artistlist.at(i).toUtf8());
        if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST, searchfor)
             && mpd_search_commit(conn))
        {
           int albumcount = 0;
           while ((pair = mpd_recv_pair_tag(conn, MPD_TAG_ALBUM)) != nullptr && albumcount < static_cast<int>(maxalbums))
           {
    ...
    if (sort_by_date) // This takes about 5 msec/album (i.e. slow)
    {
        ...
        {
            mpd_search_db_songs(conn, false);
            if (   mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM, static_cast<const char*>((iter->album).toUtf8()))
                && mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST, static_cast<const char*>((iter->artist).toUtf8()))
                && mpd_search_commit(conn))
            {
                while ((song = mpd_recv_song (conn)))
                {
                    QDateTime time;
                    uint get_last = static_cast<uint>(mpd_song_get_last_modified(song));
...
qm_itemList qm_mpdCom::get_album_searchlist(QString searchfor)
{
    ...
    mpd_search_db_songs(conn, false);
    if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM, static_cast<const char*>(searchfor.toUtf8()))
        && mpd_search_commit(conn))
    {
        while ((song = mpd_recv_song (conn)))
        {
...
qm_itemList qm_mpdCom::get_albums_for_artist(QString artist)
{
    ...
    if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST, static_cast<const char*>(artist.toUtf8()))
        && mpd_search_commit(conn))
    {
        while ((song = mpd_recv_song (conn)))
        {
...
qm_itemList qm_mpdCom::get_artist_searchlist(QString searchfor)
{
    ...
    struct mpd_song *song;
    mpd_search_db_songs(conn, false);
    if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST, static_cast<const char*>(searchfor.toUtf8()))
        && mpd_search_commit(conn))
    {
        while ((song = mpd_recv_song (conn)))
        {
...
qm_itemList qm_mpdCom::get_songlist(QString album, QString artist, QString genre)
{
    ...
    mpd_search_db_songs(conn, true);
    if (artist != "*^*")
        mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST, static_cast<const char*>(artist.toUtf8()));
    if (album != "*^*")
        mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM, static_cast<const char*>(album.toUtf8()));
    if (genre != "*^*")
        mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_GENRE, static_cast<const char*>(genre.toUtf8()));
...
qm_itemList qm_mpdCom::get_song_searchlist(QString searchfor)
{
    ...
    struct mpd_song *song;
    mpd_search_db_songs(conn, false);
    if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_TITLE, static_cast<const char*>(searchfor.toUtf8()))
        && mpd_search_commit(conn))
    {
        while ((song = mpd_recv_song (conn)))
        {
...
qm_itemList qm_mpdCom::get_genre_artistlist(QString searchfor)
{
    ...
    if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT, MPD_TAG_GENRE, static_cast<const char*>(searchfor.toUtf8()))
        && mpd_search_commit(conn))
    {
        while ((song = mpd_recv_song (conn)))
        {
...
qm_itemList qm_mpdCom::get_playlist_itemlist(QString playlist)
{
    ...
    struct mpd_song *song;
    mpd_send_list_playlist_meta(conn, static_cast<const char*>(playlist.toUtf8()));
...
void qm_mpdCom::find_config_files(bool show_dialogs)
{
    ...
    if (config_count > 1)
    {
        if (b_mpdconf_found)
        {
            QString msg = "\nMultiple MPD configuration files found! Using : " + mpdconf_path + "\n";
            printf ("%s", msg.toUtf8().constData());
            if (show_dialogs)
