...
void qm_mpdCom::set_config(qm_config *conf) { config = conf; }
...
void qm_mpdCom::configure() {
  ...
  if (v.validate(server_name, pos) == 2 && server_name != "127.0.0.1" &&
      server_name != "0.0.0.0") {
...
QString qm_mpdCom::get_musicPath() {
  ...
  if (config->mpd_config_allowed && config->mpd_socket_conn) {
    mpd_send_command(conn, "config", nullptr);
    struct mpd_pair *pair = mpd_recv_pair(conn);
    if (pair != nullptr) {
      result = pair->value;
      QString msg = "MPD's music dir : " + result + "\n";
  ...
  if (!b_success) {
    if (b_autodetect) {
      if (b_mpdconf_found) {
        QString msg = "MPD's music dir : looking in " + mpdconf_path + "\n";
        printf("%s", msg.toUtf8().constData());
...
bool qm_mpdCom::mpd_connect() {
  ...
  // no server, no go.
  if (server_name.isEmpty() &&
      b_autodetect) // DEBUG : remove "&& b_autodetect"?
  {
    server_name = "localhost";
  ...
  conn = mpd_connection_new(static_cast<const char *>(server_name.toUtf8()),
                            static_cast<uint>(server_port), 30000);
  ...
  while ((pair = mpd_recv_command_pair(conn)) != nullptr) {
  ...
  if (!server_password.isEmpty()) {
    ...
    mpd_send_password(conn,
                      static_cast<const char *>(server_password.toUtf8()));
    mpd_response_finish(conn);
    if (mpd_connection_get_error(conn) != MPD_ERROR_SUCCESS) {
      printf("Error : %s\n", mpd_connection_get_error_message(conn));
      mpd_connection_clear_error(conn);
      show_messagebox(
          tr("Provided password refused by MPD"),
          tr("Check out the 'Connect' tab in the 'settings' window."));
    } else
  ...
  if (!b_status_allowed) {
    mpd_disconnect(true);
    printf("No permission to get MPD's status. Is a (different) Password "
           "required?. Connection closed.\n");
    show_messagebox(tr("No permission to get MPD's status"),
                    tr("Possibly a (different) password is required. Check out "
                       "the 'Connect' tab in the 'settings' window."));
    return false;
  }
  ...
  if (!config->mpd_musicpath.isEmpty()) {
    ...
    if (!dir.exists()) {
      ...
      config->mpd_musicpath_status = 3; //"Configured music dir does not exist";
...
void qm_mpdCom::play_this(uint ID) {
  if (conn == nullptr)
    return;
...
void qm_mpdCom::volume_up(int i) {
  if (conn == nullptr || b_no_volume)
    return;
  int vol = current_volume + i;
  if (vol > 100)
    vol = 100;
  mpd_send_set_volume(conn, static_cast<uint>(vol));
  mpd_response_finish(conn);
...
void qm_mpdCom::set_seek(uint percent) {
  if (conn == nullptr)
    return;
  mpd_send_seek_pos(conn, static_cast<uint>(current_songpos), percent);
  mpd_response_finish(conn);
...
bool qm_mpdCom::error_check(QString action) {
  if (conn == nullptr) {
    mpd_disconnect(false);
    emit sgnl_connected(false);
    // printf ("Error : on \"" + action.toUtf8().constData() + "\" : connection
    // is NULL\n");
    QString msg = "Error : on \"" + action + "\" : connection is NULL\n";
    printf("%s", msg.toUtf8().constData());
  ...
  if (mpd_connection_get_error(conn) != MPD_ERROR_SUCCESS) {
    ...
    if (mpd_connection_get_error(conn) != MPD_ERROR_CLOSED) {
      // A "broken pipe" error means the connection is lost (which really is
      // quite fatal)
      // "reset by peer" error means the user killed mpd (which is also quite
      // fatal)
      QString errormsg = mpd_connection_get_error_message(conn);

      if (!errormsg.contains("Broken pipe", Qt::CaseInsensitive) &&
          !errormsg.contains("reset by peer", Qt::CaseInsensitive)) {
        QString msg = "Non fatal error : on \"" + action + "\": " +
                      mpd_connection_get_error_message(conn) + "\n";
        printf("%s", msg.toUtf8().constData());
        ...
        QString msg = "Connection error : on \"" + action + "\": " +
                      mpd_connection_get_error_message(conn) + "\n";
        printf("%s", msg.toUtf8().constData());
      ...
      QString msg = "Fatal error : on \"" + action + "\": " +
                    mpd_connection_get_error_message(conn) + "\n";
      printf("%s", msg.toUtf8().constData());
    }

    // proceed as 'fatal': disconnnect & reconnect
    mpd_connection_clear_error(conn);
    printf("Reconnecting...\n");
    mpd_disconnect(false);
    emit sgnl_connected(false);
...
void qm_mpdCom::set_xfade(uint secs) {
  if (conn == nullptr)
    return;
...
bool qm_mpdCom::status_check() {
  ...
  if (conn &&
      mpd_send_status(conn)) // FIXME : program crashes here when mpd was killed
  {
    servStatus = mpd_recv_status(conn);

    mpd_response_finish(conn);

    if (!error_check("mpd_recv_status") || servStatus == nullptr) {
      b_statcheck_busy = false;
  ...
  if (current_playlist <
      static_cast<int>(mpd_status_get_queue_version(servStatus))) {
    current_playlist =
        static_cast<int>(mpd_status_get_queue_version(servStatus));
  ...
  if ((current_songID != mpd_status_get_song_id(servStatus)) || b_reload) {
    ...
    if (entity == nullptr ||
        !error_check("status_check : mpd_send_current_song")) {
      current_songpos = -1;
      current_songID = -1;
      song = nullptr;
    } else {
...
void qm_mpdCom::get_songInfo_from(qm_songInfo *sInfo, mpd_song *theSong) {
  ...
  if (theSong == nullptr)
    ...
    if (sInfo->file.startsWith("http://")) {
      ...
      if (sInfo->file.startsWith(
              "/")) // URL relative to MPD music dir doesn't have a leading '/'
        sInfo->type = TP_SONGX;
      else
        sInfo->type = TP_SONG;

      sInfo->time = static_cast<int>(mpd_song_get_duration(theSong));
    }

    sInfo->songID = static_cast<int>(mpd_song_get_id(theSong));
    sInfo->songNr = static_cast<int>(mpd_song_get_pos(theSong));

    sInfo->name = QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_NAME, 0));
    sInfo->artist =
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_ARTIST, 0));
    sInfo->title =
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_TITLE, 0));
    sInfo->album =
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_ALBUM, 0));
    sInfo->track = fix_trackformat(
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_TRACK, 0)));
    ...
    sInfo->genre =
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_GENRE, 0));
    sInfo->comment =
        QString::fromUtf8(mpd_song_get_tag(theSong, MPD_TAG_COMMENT, 0));
  }
}
...
QString qm_mpdCom::get_version() {
  ...
  if (mpd_connection_get_server_version(conn)[0] == 0 &&
      mpd_connection_get_server_version(conn)[1] < 16)
...
void qm_mpdCom::reset_playlist() { current_playlist--; }
...
bool qm_mpdCom::execute_single_command(qm_mpd_command command, bool resetlist) {
  ...
  */
  ...
  case CMD_ADD: {
    ...
    if (mpd_run_add(conn, static_cast<const char *>(command.file.toUtf8()))) {
      plist_lenght++;
      result = true;
    }
    break;
  ...
  case CMD_INS: {
    ...
    if (mpd_run_add(conn, static_cast<const char *>(command.file.toUtf8()))) {
      ...
      // move into position
      current_playlist++; // prevent list update
      mpd_run_move(conn, plist_lenght - 1, static_cast<uint>(command.moveTo));
    }
    break;
  }
  ...
  case CMD_DEL: {
    // delete
    current_playlist++; // prevent list update
    if (mpd_run_delete_id(conn, static_cast<uint>(command.songid))) {
      plist_lenght--;
      result = true;
    }
    break;
...
int qm_mpdCom::execute_cmds(qm_commandList cmdlist, bool resetlist) {
  ...
  while (itr.hasNext()) {
    ...
    {
      if (mpd_send_rm(conn,
                      static_cast<const char *>(curCommand.file.toUtf8())))
        results++;
      break;
    }
    ...
    case CMD_INS: {
      ...
      if (mpd_send_add(conn,
        ...
        // move into position
        current_playlist++; // prevent list update
        mpd_send_move(conn, plist_lenght - 1,
                      static_cast<uint>(curCommand.moveTo));
      }
      break;
    }
    ...
    {
      current_playlist++; // prevent list update
      if (mpd_send_move_id(conn, static_cast<uint>(curCommand.songid),
                           static_cast<uint>(curCommand.moveTo)))
        results++;
      break;
    }
    ...
    {
      current_playlist++; // prevent list update
      if (mpd_send_move(conn, static_cast<uint>(curCommand.songnr),
                        static_cast<uint>(curCommand.moveTo)))
        results++;
      break;
    }
    ...
    case CMD_DEL: {
      current_playlist++; // prevent list update
      if (mpd_send_delete_id(conn, static_cast<uint>(curCommand.songid))) {
        results++;
        plist_lenght--;
      }
      break;
    ...
    case CMD_SCN: {
      b_dbase_updating = true;
      current_playlist++; // prevent list update
      if (mpd_send_command(conn, "rescan",
                           static_cast<const char *>(curCommand.file.toUtf8()),
                           nullptr)) {
        results++;
      }
    ...
    case CMD_UPD: {
      b_dbase_updating = true;
      current_playlist++; // prevent list update
      if (mpd_send_command(conn, "update",
                           static_cast<const char *>(curCommand.file.toUtf8()),
                           nullptr)) {
        results++;
      }
...
void qm_mpdCom::shuffle_list() {
  if (plist_lenght < 2 || conn == nullptr)
    return;
  ...
  if (current_songID != -1) {
    ...
    mpd_send_move_id(conn, static_cast<uint>(current_songID), 0);
    mpd_response_finish(conn);
...
void qm_mpdCom::get_statistics() {
  ...
  mpd_response_finish(conn);
  if (!error_check("mpd_recv_stats") || sData == nullptr) {
    emit sgnl_dbase_statistics(tr("Data not available"));
    if (sData != nullptr)
  ...
  QString statistics = "<i>" + tr("Artists") + "</i> " +
                       QString::number(mpd_stats_get_number_of_artists(sData)) +
                       "<i> &middot; " + tr("Albums") + "</i> " +
                       QString::number(mpd_stats_get_number_of_albums(sData)) +
                       "<i> &middot; " + tr("Songs") + "</i> " +
                       QString::number(mpd_stats_get_number_of_songs(sData));
...
QString qm_mpdCom::save_playlist(QString listname) {
  ...
  mpd_send_save(conn, static_cast<const char *>(listname.toUtf8()));
  mpd_response_finish(conn);
  ...
  if (!error_check("save_playlist"))
    return "Error : " +
           QString::fromUtf8(mpd_connection_get_error_message(conn));
  else
...
QString qm_mpdCom::save_selection(QStringList tobeSaved, QString listname) {
  ...
  for (int i = 0; i < tobeSaved.size(); ++i) {
    mpd_send_playlist_add(conn, static_cast<const char *>(listname.toUtf8()),
                          static_cast<const char *>(tobeSaved.at(i).toUtf8()));
  }
  mpd_command_list_end(conn);
  mpd_response_finish(conn);
...
qm_itemList qm_mpdCom::get_yearlist() {
  ...
  /*
      ...
      if ( mpd_search_db_tags(conn, MPD_TAG_ALBUM) && mpd_search_commit(conn) )
      {
          while ((pair = mpd_recv_pair_tag(conn, MPD_TAG_ALBUM)) != nullptr)
          {
              albumlist.push_back(QString::fromUtf8(pair->value));
              mpd_return_pair(conn, pair);
          }
      }
      mpd_response_finish (conn);
      ...
      for (int i = 0; i < albumlist.size(); ++i)
      {
          ...
          if ( mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT,
     MPD_TAG_ALBUM, static_cast<const char*>(albumlist.at(i).toUtf8()))
               && mpd_search_commit(conn))
          {
              while ((song = mpd_recv_song (conn)))
              {
                  if (mpd_song_get_tag (song, MPD_TAG_ARTIST, 0))
                      newartist = QString::fromUtf8(mpd_song_get_tag (song,
     MPD_TAG_ARTIST, 0));
                  else
                      newartist = "";
                  ...
                  if (mpd_song_get_tag (song, MPD_TAG_DATE, 0) != nullptr )
                      newyear = QString::fromUtf8(mpd_song_get_tag (song,
     MPD_TAG_DATE, 0));
                  else
                      newyear = "";
                  ...
                  QString newalbum = albumlist.at(i);
  ...
  */
  ...
  while ((song = mpd_recv_song(conn))) {
    ...
    if (album != newalbum || year != newyear) {
      ...
      newItem.album = newalbum;
      newItem.artist =
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_ARTIST, 0));
      if (newyear.isEmpty()) {
        newItem.sorter = "0000" + newalbum + newItem.artist;
        newItem.year = "";
      } else {
...
qm_itemList qm_mpdCom::get_albumlist(bool sort_by_date) {
  ...
  /* // DEBUG >>
      ...
      if ( mpd_search_db_tags(conn, MPD_TAG_ARTIST)
           && mpd_search_commit(conn) )
      {
          while ((pair = mpd_recv_pair_tag(conn, MPD_TAG_ARTIST)) != nullptr)
          {
              artistlist.push_back(QString::fromUtf8(pair->value));
              mpd_return_pair(conn, pair);
          }
      }
      mpd_response_finish (conn);
      ...
      {
          uint maxalbums = 512;
          char** albumlist = new char* [maxalbums];
          mpd_search_db_tags(conn, MPD_TAG_ALBUM);
          const char* searchfor = static_cast<const
  char*>(artistlist.at(i).toUtf8());
          if (mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT,
  MPD_TAG_ARTIST, searchfor)
               && mpd_search_commit(conn))
          {
             int albumcount = 0;
             while ((pair = mpd_recv_pair_tag(conn, MPD_TAG_ALBUM)) != nullptr
  && albumcount < static_cast<int>(maxalbums))
             {
                 albumlist[albumcount] = const_cast<char*>(pair->value);
                 albumcount++;
                 qm_listitemInfo newItem;
                 newItem.type = TP_ALBUM;
                 newItem.album = QString::fromUtf8(pair->value);
                 newItem.artist = artistlist.at(i);
                 newItem.sorter = newItem.album + newItem.artist;
                 itemlist.push_back(newItem);
                 mpd_return_pair(conn, pair);
             }
          }
      ...
      if (sort_by_date) // This takes about 5 msec/album (i.e. slow)
      {
          ...
          for (iter = itemlist.begin(); iter != itemlist.end(); ++iter)
          {
              mpd_search_db_songs(conn, false);
              if (   mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT,
  MPD_TAG_ALBUM, static_cast<const char*>((iter->album).toUtf8()))
                  && mpd_search_add_tag_constraint (conn, MPD_OPERATOR_DEFAULT,
  MPD_TAG_ARTIST, static_cast<const char*>((iter->artist).toUtf8()))
                  && mpd_search_commit(conn))
              {
                  while ((song = mpd_recv_song (conn)))
                  {
                      QDateTime time;
                      uint get_last =
  static_cast<uint>(mpd_song_get_last_modified(song));

                      if (get_last)
                      {
                          time.setTime_t(get_last);
                          iter->moddate = time.toString("yy/MM/dd  hh:mm");

                          // invert the sort order
                          uint oldlast = nowtime - time.toTime_t();
                          QString sort  = QString::number( oldlast );

                          while (sort.length() < nowtime_length)
                              sort = "0" + sort;

                          iter->sorter = sort + iter->sorter;
                          mpd_song_free (song);
                          break;
                      }
                      else
                          mpd_song_free (song); // and try the next one
                }
  ...
  mpd_search_db_songs(conn, false);
  if (mpd_search_add_tag_constraint(conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM,
                                    "") &&
      mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
      if (mpd_song_get_tag(song, MPD_TAG_ALBUM, 0) != nullptr)
      ...
      if (album != newalbum || artist != newartist) {
        ...
        if (sort_by_date) {
          ...
          time.setTime_t(static_cast<uint>(mpd_song_get_last_modified(song)));
    ...
    printf("get_albumlist failed at mpd_search_commit\n");
...
qm_itemList qm_mpdCom::get_album_searchlist(QString searchfor) {
  ...
  mpd_search_db_songs(conn, false);
  if (mpd_search_add_tag_constraint(
          conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM,
          static_cast<const char *>(searchfor.toUtf8())) &&
      mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
      if (mpd_song_get_tag(song, MPD_TAG_ARTIST, 0) != nullptr)
        newartist =
...
qm_itemList qm_mpdCom::get_albums_for_artist(QString artist) {
  ...
  if (mpd_search_add_tag_constraint(
          conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST,
          static_cast<const char *>(artist.toUtf8())) &&
      mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
    ...
    if (mpd_song_get_tag(song, MPD_TAG_ARTIST, 0) != nullptr) {
      ...
      if (artist != newartist) {
        ...
        if (config->ignore_leading_the &&
            (newItem.sorter).startsWith("the ", Qt::CaseInsensitive))
      ...
      if (mpd_song_get_tag(song, MPD_TAG_ARTIST, 0) != nullptr) {
        ...
        if (artist != newartist) {
          ...
          if (config->ignore_leading_the &&
              (newItem.sorter).startsWith("the ", Qt::CaseInsensitive))
            newItem.sorter =
                (newItem.sorter).right((newItem.sorter).size() - 4);
          itemlist.push_back(newItem);
          artist = newartist;
        }
      } else if (searchfor.isEmpty()) {
...
void qm_mpdCom::get_allsubdirs(QString filePath, QString searchfor) {
  ...
  mpd_send_list_meta(conn, static_cast<const char *>(filePath.toUtf8()));
  ...
  while ((entity = mpd_recv_entity(conn)) != nullptr) {
    if (mpd_entity_get_type(entity) == MPD_ENTITY_TYPE_DIRECTORY) {
      ...
      newItem.artist =
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_ARTIST, 0));
      newItem.album =
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_ALBUM, 0));
      newItem.title =
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_TITLE, 0));
      newItem.track = fix_trackformat(
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_TRACK, 0)));
      newItem.genre =
          QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_GENRE, 0));
      itemlist.push_back(newItem);
...
qm_itemList qm_mpdCom::get_songlist(QString album, QString artist,
                                    QString genre) {
  qm_itemList itemlist;
  ...
  mpd_search_db_songs(conn, true);
  if (artist != "*^*")
    mpd_search_add_tag_constraint(conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ARTIST,
                                  static_cast<const char *>(artist.toUtf8()));
  if (album != "*^*")
    mpd_search_add_tag_constraint(conn, MPD_OPERATOR_DEFAULT, MPD_TAG_ALBUM,
                                  static_cast<const char *>(album.toUtf8()));
  if (genre != "*^*")
    mpd_search_add_tag_constraint(conn, MPD_OPERATOR_DEFAULT, MPD_TAG_GENRE,
                                  static_cast<const char *>(genre.toUtf8()));
  ...
  if (mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
      ...
      if (newItem.track.size() == 0)
        newItem.sorter += "000";
      else if (newItem.track.size() == 1)
        newItem.sorter += "00" + newItem.track;
      else if (newItem.track.size() == 2)
  ...
  printf("Adding %s items to playlist\n",
         QString::number(numitems).toUtf8().constData());
  return itemlist;
}
...
qm_itemList qm_mpdCom::get_song_searchlist(QString searchfor) {
  ...
  mpd_search_db_songs(conn, false);
  if (mpd_search_add_tag_constraint(
          conn, MPD_OPERATOR_DEFAULT, MPD_TAG_TITLE,
          static_cast<const char *>(searchfor.toUtf8())) &&
      mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
      ...
      if (mpd_song_get_tag(song, MPD_TAG_TITLE, 0) != nullptr) {
        ...
        if (mpd_song_get_tag(song, MPD_TAG_TRACK, 0)) {
          newItem.track = fix_trackformat(
              QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_TRACK, 0)));
          newItem.sorter =
              newItem.artist + newItem.album + newItem.track + newItem.title;
        } else
          newItem.sorter =
              newItem.artist + newItem.album + "00" + newItem.title;
        if (newItem.title.isEmpty())
...
qm_itemList qm_mpdCom::get_genre_artistlist(QString searchfor) {
  ...
  if (mpd_search_add_tag_constraint(
          conn, MPD_OPERATOR_DEFAULT, MPD_TAG_GENRE,
          static_cast<const char *>(searchfor.toUtf8())) &&
      mpd_search_commit(conn)) {
    while ((song = mpd_recv_song(conn))) {
      QString genre =
...
qm_itemList qm_mpdCom::get_genrelist(QString searchfor) {
  ...
  if (searchfor == "*^*") // get full list
  {
    QString newgenre;
    struct mpd_pair *pair;
    if (mpd_search_db_tags(conn, MPD_TAG_GENRE) && mpd_search_commit(conn)) {
    ...
    if (mpd_search_add_tag_constraint(
      ...
      while ((song = mpd_recv_song(conn))) {
        ...
        if (mpd_song_get_tag(song, MPD_TAG_GENRE, 0) != nullptr) {
          ...
          newItem.genre =
              QString::fromUtf8(mpd_song_get_tag(song, MPD_TAG_GENRE, 0));
          newItem.sorter = newItem.genre; // not toLower() or libview mays show
                                          // things like: Jazz jazz Jazz
          itemlist.push_back(newItem);
        } else if (searchfor.isEmpty()) {
          qm_listitemInfo newItem;
          newItem.genre = "";
...
void qm_mpdCom::reset_current_song() { current_songID = -1; }
...
qm_itemList qm_mpdCom::get_playlist_itemlist(QString playlist) {
  ...
  struct mpd_song *song;
  mpd_send_list_playlist_meta(conn,
                              static_cast<const char *>(playlist.toUtf8()));
...
int qm_mpdCom::get_listchanger() {
  ...
  mpd_entity *entity = nullptr;
  mpd_send_queue_changes_meta(conn, static_cast<uint>(current_playlist - 1));
  while ((entity = mpd_recv_entity(conn)) != nullptr) {
...
void qm_mpdCom::set_replaygain_mode(int mode) {
  ...
  mpd_send_command(conn, "replay_gain_mode", rg_mode, nullptr);
  mpd_response_finish(conn);
...
void qm_mpdCom::set_sInfo(qm_songInfo *sInf) { current_songinfo = sInf; }
...
void qm_mpdCom::find_config_files(bool show_dialogs) {
  ...
  if (config_count > 1) {
    if (b_mpdconf_found) {
      QString msg = "\nMultiple MPD configuration files found! Using : " +
                    mpdconf_path + "\n";
      printf("%s", msg.toUtf8().constData());
      if (show_dialogs)
        show_messagebox(tr("Multiple MPD configuration files found"),
                        allConfigs + "\n\n" + tr("The default choice is : ") +
                            mpdconf_path + "\n\n" +
                            tr("Please make sure that MPD is also using this. "
                               "You can set MPD to use a specific "
                               "configuration file in /etc/default/mpd."));
    } else {
      printf("\nMultiple MPD configuration files found, but none are readable");
      if (show_dialogs)
        show_messagebox(
            tr("Multiple MPD configuration files found, but none are readable"),
            allConfigs + "\n\n" +
                tr("Please create a readable configuration file that MPD is "
                   "also using or create a custom connection profile in the "
                   "'Connect' tab of the settings window."));
    }
    ...
    if (!b_mpdconf_found) {
      ...
      if (show_dialogs)
        show_messagebox(
            tr("MPD configuration file found, but it is not readable"),
            allConfigs + "\n\n" +
                tr("Please create a readable configuration file that MPD is "
                   "also using or create a custom connection profile in the "
                   "'Connect' tab of the settings window."));
  ...
  } else // (config_count == 0)
  {
    ...
    if (show_dialogs)
      show_messagebox(tr("Auto configuration did not succeed"),
                      tr("Please create a custom connection profile in the "
                         "'Connect' tab of the settings window."));
  }
}
