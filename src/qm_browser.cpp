...
void qm_browser::setupUI() {
  ...
  lib_view->setVerticalScrollBarPolicy(
  ...
  plist_view->setVerticalScrollBarPolicy(
      Qt::ScrollBarAlwaysOn); // avoid problems with viewport()->width();
  ...
  QObject::connect(sloader, SIGNAL(streamdownload_done(QStringList)), this,
                   SLOT(on_streamdownload_done(QStringList)));
  QObject::connect(cb_dbasemode, SIGNAL(activated(int)), this,
                   SLOT(on_dbase_newmode(int)));
  QObject::connect(cb_searchmode, SIGNAL(activated(int)), this,
                   SLOT(set_searchmode(int)));
  QObject::connect(cb_selectmode, SIGNAL(activated(int)), this,
                   SLOT(set_selectmode(int)));
  QObject::connect(bt_search, SIGNAL(clicked()), this, SLOT(search_in_lib()));
  QObject::connect(le_search, SIGNAL(returnPressed()), this,
                   SLOT(search_in_lib()));
  QObject::connect(bt_select, SIGNAL(clicked()), this, SLOT(select_in_plist()));
  QObject::connect(le_select, SIGNAL(returnPressed()), this,
                   SLOT(select_in_plist()));
  ...
  QObject::connect(bt_options, SIGNAL(clicked()), this,
                   SLOT(on_options_clicked()));
  QObject::connect(plist_view, SIGNAL(sgnl_plist_statistics(QString)), this,
                   SLOT(set_plist_stats(QString)));
  QObject::connect(plist_view, SIGNAL(sgnl_plist_dropreceived(int)), lib_view,
                   SLOT(on_plist_dropreceived(int)));
  QObject::connect(bt_library, SIGNAL(clicked()), this,
                   SLOT(on_library_toggle()));
  QObject::connect(bt_playlist, SIGNAL(clicked()), this,
                   SLOT(on_playlist_toggle()));
  QObject::connect(theSplitter, SIGNAL(splitterMoved(int, int)), this,
                   SLOT(splittermoved()));
} // setupUi
...
void qm_browser::get_the_stream() {
  ...
  QString input = QInputDialog::getText(
      this, tr("Enter URL"), tr("Location of the stream's playlist file:"),
...
void qm_browser::on_streamdownload_done(QStringList result) {
  ...
  if (result.value(0) == "nothing") {
    QString message =
        tr("Expected playlist file was not found.\nDownload aborted.");
    QMessageBox::warning(this, tr("Error"), message);
    return;
  }

  if (result.value(0) == "timeout") {
    QString message = tr("Download takes too long:") + " " + result.value(1) +
                      " " + tr("% in 10 sec.\nDownload aborted.");
    QMessageBox::warning(this, tr("Error"), message);
    return;
  }
  ...
  QString message = tr("Downloaded file:") + " " + result.value(1) + "\n" +
                    ...
                    tr("items added to the playlist.");
...
void qm_browser::set_searchmode(int i) { config->browser_searchmode = i; }

void qm_browser::set_selectmode(int i) { config->browser_selectmode = i; }
...
void qm_browser::on_options_clicked() { options_menu->exec(QCursor::pos()); }
...
void qm_browser::set_connected(qm_mpdCom *conn, bool isconnected) {
  ...
  if (b_mpd_connected) {
    QObject::connect(mpdCom, SIGNAL(sgnl_dbase_statistics(QString)), this,
                     SLOT(set_dbase_stats(QString)));
    QObject::connect(mpdCom, SIGNAL(sgnl_update_ready()), this,
                     SLOT(on_update_ready()));
...
void qm_browser::resizeEvent(QResizeEvent *rse) {
  ...
  QList<int> List;
  int new_left = (rse->size().width() * theSplitter->sizes().value(0)) /
                 rse->oldSize().width();
  List.append(new_left);
  int new_right = (rse->size().width() * theSplitter->sizes().value(1)) /
                  rse->oldSize().width();
