...
void qm_stream_loader::on_download_progress(qint64 bytesReceived,
                                            qint64 bytesTotal) {
  if (download_time.elapsed() > 10000) // msecs
  {
    int percent;
    if (bytesTotal > 0)
      percent = static_cast<int>(100 * (static_cast<double>(bytesReceived) /
...
void qm_stream_loader::on_download_finished(QNetworkReply *reply) {
  ...
  if (reply->error())
    ...
    if (!reply->isReadable())
      ...
      } else if (qba.startsWith("[playlist]")) {
        result << "ok"
               << "pls";
        QTextStream in(qba);
        while (!in.atEnd()) {
      ...
      } else if (qba.startsWith("<?xml") && qba.indexOf("xspf.org") != -1) {
        result << "ok"
               << "xspf";
        QTextStream in(qba);
        while (!in.atEnd()) {
      ...
      } else if (qba.indexOf("#EXTM3U") != -1) // m3u (with comments)
      {
        result << "ok"
               << "m3u";
        QTextStream in(qba);
      ...
      } else if (qba.indexOf("<html") == -1 &&
                 qba.indexOf("http://") != -1) // flat text list?
      {
        result << "ok"
               << "flat text m3u";
        QTextStream in(qba);
  ...
  if (counter == 0) {
    result.clear();
    result << "nothing"
           << " No streams found" << (reply->url()).toString();
  }
  emit streamdownload_done(result);
  reply->deleteLater();
}
