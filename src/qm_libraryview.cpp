...
void qm_libview::on_selection_changed() {
  ...
  while (*itr) {
    ...
    if (current->isSelected()) {
      ...
      else { // no selection if if a parent is already selected
  ...
  if (counter == 1 && currentItem() != nullptr) {
...
void qm_libview::expand_album(QTreeWidgetItem *active_item) {
  ...
  if (config->browser_libmode == LB_ID_GENRE)
    itemlist = mpdCom->get_songlist(
        active_item->getalbum, active_item->getartist, active_item->getgenre);
  else
    itemlist = mpdCom->get_songlist(active_item->getalbum,
                                    active_item->getartist, "*^*");
  ...
  for (iter = itemlist.begin(); iter != itemlist.end(); ++iter) {
    if (iter->type == TP_SONG) {
      if (iter->title.isEmpty()) // DEBUG what about iter->name?
      {
        // use file name as title
        int i = (iter->file).lastIndexOf('/') + 1;
        iter->title = (iter->file).right((iter->file).length() - i);
...
void qm_libview::set_connected(qm_mpdCom *conn, bool isconnected) {
  ...
  if (b_mpd_connected) {
    QObject::connect(mpdCom, SIGNAL(sgnl_plistsaved()), this,
                     SLOT(on_mpd_plistsaved()));
    reload_root();
...
void qm_libview::add_songdata_toItem(QTreeWidgetItem *theItem,
                                     qm_listitemInfo data) {
...
void qm_libview::on_append() { modify_playlist(false, false); }
...
void qm_libview::on_delete() {
  ...
  if (QMessageBox::warning(
          this, tr("Warning"),
          tr("This will permanently remove the selected playlists!"),
          tr("Delete"), tr("Cancel"), QString::null, 1, 0))
    return;
...
void qm_libview::on_update() {
  if (topLevelItemCount() == 0 || !b_mpd_connected ||
      (!config->mpd_rescan_allowed && !config->mpd_update_allowed))
    return;
  ...
  while (itr.hasNext()) {
    ...
    case TP_ALBUM: {
      if (config->browser_libmode == LB_ID_GENRE)
        itemlist =
            mpdCom->get_songlist(currentItem->getalbum, currentItem->getartist,
                                 currentItem->getgenre);
      else
        itemlist = mpdCom->get_songlist(currentItem->getalbum,
  ...
  for (iter = updateFilesList.begin(); iter != updateFilesList.end(); ++iter) {
    ...
    */
    ...
    if (newCommand.file != previous_dir) {
      if (config->mpd_rescan_allowed) {
        newCommand.cmd =
            CMD_SCN; // only updates files, even if timestamp has not changed
        theUpdateList.push_back(newCommand);
      }
      if (config->mpd_update_allowed) {
        newCommand.cmd = CMD_UPD; // finds additions/removals and files with
                                  // changed timestamp
        theUpdateList.push_back(newCommand);
      }
...
void qm_libview::modify_playlist(bool b_newlist, bool b_insert) {
  ...
  while (itr.hasNext()) {
    ...
    case TP_ARTIST: {
      if (config->browser_libmode == LB_ID_GENRE)
        append_to_modlist("*^*", currentItem->getartist, currentItem->getgenre,
                          b_insert);
      else
    ...
    case TP_ALBUM: {
      if (config->browser_libmode == LB_ID_GENRE)
        append_to_modlist(currentItem->getalbum, currentItem->getartist,
                          currentItem->getgenre, b_insert);
      else
        append_to_modlist(currentItem->getalbum, currentItem->getartist, "*^*",
                          b_insert);
      break;
...
void qm_libview::append_to_modlist(QString album, QString artist, QString genre,
                                   bool b_insert) {
...
void qm_libview::append_to_modlist_dir(QString path, bool b_insert) {
  ...
  for (iter = itemlist.begin(); iter != itemlist.end(); ++iter) {
    ...
    else if (iter->type == TP_SONG || iter->type == TP_STREAM) {
      qm_mpd_command newCommand;
      if (b_insert)
        newCommand.cmd = CMD_INS;
      else
...
void qm_libview::columnResized(int colnr, int oldsize, int newsize) {
  // column 0 is reset to default width 100 at startup.
  // we don't want to write that to config.
  if (colnr == 0 && newsize == 100 && !b_setup_done) {
    // just pass it on
    b_setup_done = true;
...
void qm_libview::set_panel_maxmode(bool ismax) {
  ...
  if (b_panel_max) {
    ...
    } else if (column_count == 2) // set 1st only
    {
      setColumnWidth(0, config->lib_max_col0);
    }
  } else {
