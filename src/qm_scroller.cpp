...
void qm_Scroller::pause(bool ps) { b_pause = ps; }
...
void qm_Scroller::scrollerstep() {
  ...
  for (int h = 0; h < H_display; h++) {
    for (int w = 0; w < W_display; w++) {
      ...
      // fade in & out
      if (w < 24) {
        uchar r = static_cast<uchar>((w * qRed(px) + (24 - w) * cR) / 24);
        uchar g = static_cast<uchar>((w * qGreen(px) + (24 - w) * cG) / 24);
        uchar b = static_cast<uchar>((w * qBlue(px) + (24 - w) * cB) / 24);
        px = qRgb(r, g, b);
      }
      if (w > W_display - 24) {
        int x = W_display - w;
        uchar r = static_cast<uchar>((x * qRed(px) + (24 - x) * cR) / 24);
        uchar g = static_cast<uchar>((x * qGreen(px) + (24 - x) * cG) / 24);
        uchar b = static_cast<uchar>((x * qBlue(px) + (24 - x) * cB) / 24);
        px = qRgb(r, g, b);
      }
