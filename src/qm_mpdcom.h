...
#if (defined(LIBMPDCLIENT_MAJOR_VERSION) && LIBMPDCLIENT_MAJOR_VERSION > 1) && \
    (defined(LIBMPDCLIENT_MINOR_VERSION) && LIBMPDCLIENT_MINOR_VERSION > 11)
...
private:
  ...
  bool b_statcheck_busy, b_mpdconf_found, b_dbase_updating, b_connecting,
      b_reload, b_no_volume, b_remote_server, b_autodetect;

  int current_playlist, current_songpos, current_songID, current_status,
      current_volume;

  uint plist_lenght, server_port;

  QString server_name, server_password, mpdconf_path;
};
