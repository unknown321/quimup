...
void qm_settings::retranslateUI()
{
    ...
    QString s_tips     = "<font face=\"Sans Serif\"><p align=\"center\"><br><b>Q</b>t <b>U</b>ser <b>I</b>nterface for the <b>MU</b>sic <b>P</b>layer daemon<br><br>© 2008-2018 Johan Spee<br>quimup@jhanalog.com<br><br><font color=\"darkblue\">";
            s_tips    += tr("License")
                       ...
                       + tr("To show albumart or use external programs Quimup must be able to access MPD's <b>music directory</b> (the current status of which is shown in the 'Connect' tab). The directory is requested directly from MPD 0.17 but with earlier versions must be read from mpd.conf. To avoid problems set MPDCONF to <i>your mpd.conf</i> in /etc/default/mpd.")
                       + "<br><br>"
                       + tr("To make sure you have all necessary <b>user rights</b> it is recommended to NOT run MPD as a service (systemctl) and let Quimup start MPD (see the 'Server' tab).")
                       + "<br><br>"
                       + tr("When Quimup can access the music directory it can also get the <b>modification dates</b> (when sorting albums by time-stamp) directly from the files. This is slower than getting the info from MPD but avoids errors.")
                       + "<br><br>"
