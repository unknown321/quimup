...
void qm_settings::setupUI() {
  ...
  cm_profiles->insertItems(0, QStringList() << "0"
                                            << "1"
                                            << "2"
                                            << "3"
                                            << "4");
  ...
  QObject::connect(cm_profiles, SIGNAL(currentIndexChanged(int)), this,
                   SLOT(set_profile(int)));
  QObject::connect(bt_connect, SIGNAL(clicked()), this, SLOT(server_connect()));
  QObject::connect(bt_fonts_reset, SIGNAL(clicked()), this,
                   SLOT(reset_fonsizes()));
  QObject::connect(bt_cust_color, SIGNAL(clicked()), this,
                   SLOT(show_colorpicker()));
  QObject::connect(colorpicker, SIGNAL(sgnl_newcolors(QColor, QColor)), this,
                   SLOT(on_new_customcolors(QColor, QColor)));
} // setupUi
...
void qm_settings::retranslateUI() {
  if (config->show_tooltips) {
    ...
    le_host_text->setToolTip(
        tr("\"/home/USER/.mpd/socket\", \"localhost\", or a network address"));
    le_killcommand->setToolTip("<nobr>\"mpd --kill\"</nobr> " +
                               tr("or \"killall mpd\""));
    ...
    lb_xfade->setToolTip(
        tr("Turn on-off in the media browser window (options menu)"));
    bt_close->setToolTip(tr("Close this window (does not apply the settings)"));
    bt_apply->setToolTip(
        tr("Apply current settings (does not close this window)"));
    bt_connect->setToolTip(tr("Apply profile and connect to MPD"));
    lb_musicdir_icon->setToolTip(
        tr("Accessibility status of MPD's music directory"));
  ...
  cb_autoConn->setText(tr("Connect on start-up"));
  ...
  lb_xfade->setText(tr("Time (sec).") + "<i> " + tr("Turn on-off in playlist") +
                    "</i>");
  ...
  cb_auto_playfirst->setText(
      tr("Start first song after 'Open in new playlist'"));
  ...
  QString s_tips =
      "<font face=\"Sans Serif\"><p align=\"center\"><br><b>Q</b>t <b>U</b>ser "
      "<b>I</b>nterface for the <b>MU</b>sic <b>P</b>layer daemon<br><br>© "
      "2008-2018 Johan Spee<br>quimup@jhanalog.com<br><br><font "
      "color=\"darkblue\">";
  s_tips +=
      tr("License") + "<div align=\"justify\"> <font size=\"-1\">" +
      "This program is free software. You can redistribute it and/or modify it "
      "under the terms of the GNU General Public License as published by the "
      "Free Software Foundation; either version 2 of the License, or (at your "
      "option) any later version. This program is distributed in the hope that "
      "it will be useful, but without any warrenty; without even the implied "
      "warranty of merchantability or fitness for a particular purpose. See "
      "the GNU General Public License for details." +
      "</font></font><br></div><b>" + tr("Tips") +
      "</b><br> <div align=\"justify\">" +
      tr("To show albumart or use external programs Quimup must be able to "
         "access MPD's <b>music directory</b> (the current status of which is "
         "shown in the 'Connect' tab). The directory is requested directly "
         "from MPD 0.17 but with earlier versions must be read from mpd.conf. "
         "To avoid problems set MPDCONF to <i>your mpd.conf</i> in "
         "/etc/default/mpd.") +
      "<br><br>" +
      tr("To make sure you have all necessary <b>user rights</b> it is "
         "recommended to NOT run MPD as a service (systemctl) and let Quimup "
         "start MPD (see the 'Server' tab).") +
      "<br><br>" + tr("When Quimup can access the music directory it can also "
                      "get the <b>modification dates</b> (when sorting albums "
                      "by time-stamp) directly from the files. This is slower "
                      "than getting the info from MPD but avoids errors.") +
      "<br><br>" + tr("Right-clicking the player brings up a menu where you "
                      "can start <b>external programs</b> that can be set "
                      "here, in the Client tab. Note that the programs must be "
                      "able to access MPD's music directory (see above).") +
      "<br><br>" + tr("Clicking the time counter toggles the <b>time mode</b>, "
                      "i.e. <i>time elapsed</i> and <i>time remaining</i>.") +
      "<br><br>" + tr("Clicking the middle mouse-button on the system-tray "
                      "icon toggles <b>play / pause.") +
      "</b><br><br>"
      /*                       + tr("The <b>volume</b> can be controlled with
         the mouse wheel when the cursor is on the system-tray icon - and by
         using the slider in the player.")
                             + "<br><br>"
      */
      + tr("Pressing the spacebar in the playlist scrolls the <b>current "
           "song</b> into view.") +
      "<br><br>" + tr("A <b>scrolling title</b> speeds up (quite a bit) when "
                      "the mouse cursor is moved over it.") +
      "<br><br>" +
      tr("Quimup first checks the audio-file for embedded <b>Album-art</b>. If "
         "that fails it checks the directory where  the audio file is located "
         "for a bitmap that matches ") +
      " <i>*albumart*, *folder*, *front*, *cover* or *</i> " +
      tr("(in that order). Note that Quimup must be able to access MPD's music "
         "directory (see above).") +
      "<br><br>" +
      tr("<b>Selecting</b> an item in the library tree will disable the "
         "selection of subitems. A selected artist's albums, for instance, can "
         "not be selected because the artist already includes those albums.") +
      "<br><br>" +
      tr("Quimup will accept files that are <b>drag & drop</b>ped on the "
         "player or the playlist, or that are added using '<b>open with</b>' "
         "in your file manager - but only if it is connected to MPD using a "
         "socket (both <i>bind_to_address</i> in mpd.conf and <i>host</i> in "
         "Quimup's connection profile must point to "
         "<i>/home/USER/.mpd/socket</i>).") +
      "<br><br>" + tr("Run <i>quimup -help</i> in a terminal for a list of the "
                      "<b>command line parameters</b>.") +
      "<br><br></div><b><font color=\"darkblue\">" + tr("Enjoy!") +
      "<br><br></b></p></font></font>";
...
void qm_settings::cancel_changes() { this->close(); }
...
void qm_settings::apply_settings() {
  ...
  config->xfade_time = static_cast<uint>(sb_xfade->value());
  if (config->xfade_is_on && b_mpd_connected)
    mpdCom->set_xfade(static_cast<uint>(config->xfade_time));
  ...
  case 1: {
    ...
    config->MPD_port_1 = (le_port_text->text()).toUInt();
  ...
  case 2: {
    ...
    config->MPD_port_2 = (le_port_text->text()).toUInt();
  ...
  case 3: {
    ...
    config->MPD_port_3 = (le_port_text->text()).toUInt();
  ...
  case 4: {
    ...
    config->MPD_port_4 = (le_port_text->text()).toUInt();
  ...
  if (rb_rpg_off->isChecked() && config->rpgain_mode != "off") {
    ...
    b_set_rpgain = true;
  } else if (rb_rpg_track->isChecked() && config->rpgain_mode != "track") {
    config->rpgain_mode = "track";
    b_set_rpgain = true;
  } else if (rb_rpg_album->isChecked() && config->rpgain_mode != "album") {
    config->rpgain_mode = "album";
    b_set_rpgain = true;
  } else if (rb_rpg_auto->isChecked() && config->rpgain_mode != "auto") {
...
void qm_settings::set_config(qm_config *conf) {
  ...
  bgcolor_album.setHsv(bgcolor_album.hue(), bgcolor_album.saturation() / 3,
                       200);
  ...
  sb_xfade->setValue(static_cast<int>(config->xfade_time));
  ...
  else if (config->rb_qmcool_checked)
  ...
  else if (config->rpgain_mode == "album")
    rb_rpg_album->setChecked(true);
  else if (config->rpgain_mode == "auto")
...
void qm_settings::set_connected(qm_mpdCom *conn, bool isconnected) {
  ...
  if (!b_mpd_connected) {
    ...
    lb_conn_status_text->setText(tr("Connected to MPD") + " " +
                                 mpdCom->get_version());
...
void qm_settings::set_replaygain_mode() {
  ...
  } else if (config->rpgain_mode == "track") {
...
qm_settings::~qm_settings() { delete (colorpicker); }
