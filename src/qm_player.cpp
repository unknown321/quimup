...
qm_player::qm_player()
{
    ...
    if (config->auto_connect)
    {
        ...
        if (!mpdCom->mpd_connect())
        {
            ...
            // mpdCom->show_messagebox(tr("Could not connect to MPD!"), tr("Please check the 'Connect' tab in the 'Settings' window."));
        }
    }
    else
    {
...
void qm_player::init_vars()
{
    ...
    if (config->use_trayicon)
    {
        ...
        //the_trayicon->installEventFilter(new qm_eventHandler(this)); // No events are filtered ever
...
{
    if (config->mpd_socket_conn && e->source() == nullptr &&  e->mimeData()->hasUrls())
        e->accept();
    else
        e->ignore();
}
        ...
        if (mpgfile.isValid())
        {
            if (mpgfile.ID3v2Tag()) // has ID3v2 tag
            {
                ...
                if(!flist.isEmpty())
                {
                    ...
                    if ( image.loadFromData(reinterpret_cast<const uchar *>(pframe->picture().data()), static_cast<int>(pframe->picture().size()) ) )
                    {
            ...
            if( mpgfile.APETag())
            {
                ...
                if (listMap.contains("COVER ART (FRONT)"))
                {
                   ...
                   if (++pos > 0)
                   {
                       const TagLib::ByteVector & picvect = item.mid(static_cast<uint>(pos));
                       QByteArray image_data( picvect.data(), static_cast<int>(picvect.size()) );
        ...
        if(oggfile.isValid())
        {
            if (oggfile.tag()) // has XiphComment tag
            {
                ...
                if(!picList.isEmpty())
                {
                    ...
                    QByteArray image_data( fpic->data().data(), static_cast<int>(fpic->data().size()) );
                ...
                if (!block.isEmpty())
                {
                    ...
                    for( TagLib::StringList::ConstIterator i = block.begin(); i != block.end(); ++i )
                    {
                        ...
                        TagLib::ByteVector tdata( data.data(), static_cast<uint>(data.size()) );
        ...
        if (mp4file.isValid())
        {
            if (mp4file.tag()) // has MP4 tag
            {
                ...
                if (tag->itemListMap().contains("covr"))
                {
                    ...
                    if ( image.loadFromData(reinterpret_cast<const uchar *>(coverArt.data().data()), static_cast<int>(coverArt.data().size())) )
                    {
            ...
            if (asffile.tag()) // has asf tag
            {
                ...
                if (attrListMap.contains("WM/Picture"))
                {
                    ...
                    if (!attrList.isEmpty())
                    {
                        ...
                        if (pic.isValid())
                        {
                            const QByteArray image_data( pic.picture().data(), static_cast<int>(pic.picture().size()) );
