...
qm_plistview::qm_plistview(QWidget *parent) {
  ...
  QObject::connect(this, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)),
                   this, SLOT(on_item_dclicked(QTreeWidgetItem *)));
  ...
  QObject::connect(a_playreset, SIGNAL(triggered()), this,
                   SLOT(reset_played()));
  ...
  /*    context_menu->addSeparator();
      ...
      QObject::connect( a_loadlist, SIGNAL( triggered() ), this, SLOT(
     reload_playlist() ) );
...
void qm_plistview::fix_header() {
  ...
  plisthview->setMinimumSectionSize(20);
...
void qm_plistview::set_playlist_stats(int numTracks, int TotalTime) {
  QString plist_stats = "<i>" + tr("Tracks") + "</i> " +
                        QString::number(numTracks) + " &middot; <i>" +
                        tr("Total Time") + "</i> " + into_time(TotalTime);
...
void qm_plistview::on_item_dclicked(QTreeWidgetItem *item) {
  ...
  if (itemID != -1 && b_mpd_connected) {
    ...
    // verify external file still exsits
    if (item->get_type == TP_SONGX) {
      QFileInfo Finfo(item->get_file);
      if (!Finfo.exists()) {
        QMessageBox msgBox;
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.setText(tr("<b>File not found</b>"));
        msgBox.setInformativeText(tr("Remove the item from the playlist?"));
        msgBox.setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
        msgBox.setDefaultButton(QMessageBox::Ok);
        printf("Play: external file was not found!\n");
        if (msgBox.exec() == QMessageBox::Ok) {
          qm_mpd_command newCommand;
          newCommand.cmd = CMD_DEL;
          newCommand.songid = itemID;
    ...
    mpdCom->play_this(static_cast<uint>(itemID));
  }
}
    ...
    case STATE_PLAYING: {
      ...
      if (config->mark_played) {
        if (topLevelItem(current_songPos)->get_type == TP_SONG)
          topLevelItem(current_songPos)
              ->setIcon(1, QIcon(":/tr_trackplayed.png"));
        else {
          if (topLevelItem(current_songPos)->get_type == TP_SONGX)
            topLevelItem(current_songPos)
                ->setIcon(1, QIcon(":/tr_trackx_played.png"));
          else
            topLevelItem(current_songPos)
                ->setIcon(1, QIcon(":/tr_streamplayed.png"));
        }
        for (int i = 0; i < 6; i++)
          topLevelItem(current_songPos)
              ->setForeground(i, QBrush(col_played_fg));
...
void qm_plistview::on_mpd_newlist(qm_songinfo newPlaylist, int changerID) {
  ...
  for (iter = newPlaylist.begin(); iter != newPlaylist.end(); ++iter) {
    ...
    if (!iter->nosong) {
      ...
      } else if (iter->type == TP_SONG ||
                 iter->type == TP_SONGX) // should always be true
      {
        ...
        labels << " " + QString::number(pos) << "" // icon
               << " " + iter->artist + " "
               << " " + iter->title + " "
               << " " + into_time(iter->time) + " "
               << " " + iter->album + " : " + track + " ";
  ...
  if (column == 0)
    for (int colm = 2; colm < 6; colm++) {
      QList<QTreeWidgetItem *> result =
          findItems(findThis, Qt::MatchContains | Qt::MatchFixedString, colm);
    ...
    QList<QTreeWidgetItem *> result =
        findItems(findThis, Qt::MatchContains | Qt::MatchFixedString, column);
    QListIterator<QTreeWidgetItem *> it(result);
    while (it.hasNext()) {
      it.next()->setSelected(true);
    }
...
void qm_plistview::save_it() {
  ...
  QString listname = QInputDialog::getText(
      this, tr("Save"), tr("Enter a name for the playlist"), QLineEdit::Normal,
      suggested_name, &ok);
...
void qm_plistview::save_selected() {
  ...
  QString listname = QInputDialog::getText(this, tr("Save"),
                                           tr("Enter a name for the playlist"),
                                           QLineEdit::Normal, "selection", &ok);
  if (!ok || listname.isEmpty())
    return;
...
void qm_plistview::set_connected(qm_mpdCom *conn, bool isconnected) {
  ...
  if (b_mpd_connected) {
    QObject::connect(mpdCom, SIGNAL(sgnl_plistupdate(qm_songinfo, int)), this,
                     SLOT(on_mpd_newlist(qm_songinfo, int)));
...
void qm_plistview::dragMoveEvent(QDragMoveEvent *e) {
  if (e->source() == nullptr) // drag from outside the app
  {
...
void qm_plistview::dragEnterEvent(QDragEnterEvent *e) {
  if (e->source() != nullptr) // if drag from within application
  {
    line->show();
    e->accept();
  } else {
...
void qm_plistview::on_open_with_droprequest(QDropEvent *e) {
  ...
  // Should not happen: these go to dropEvent(QDropEvent *e)
  if (e->source() != nullptr) {
    e->ignore();
    return;
  }
...
void qm_plistview::on_open_with_request(const QString &inlist) {
  if (!config->mpd_socket_conn || !b_mpd_connected || mpdCom == nullptr)
    return;

  QString uri = inlist;
  printf("Request to open external files\n"); // DEBUG
  ...
  QStringList split_uri =
      uri.split("file://", QString::SkipEmptyParts, Qt::CaseInsensitive);
  int count = 0;
  ...
  for (iter = checked_uri.begin(); iter != checked_uri.end(); ++iter) {
    QString url = *iter;
    if (!url.startsWith("file://", Qt::CaseInsensitive)) // fixme : should never
                                                         // occur. See split,
                                                         // above.
      url = "file://" + url;
    qm_mpd_command newCommand;
    ...
    count++;
    if (count == last)
      mpdCom->execute_single_command(
          newCommand, true); // last one triggers a playlist update
    else
      mpdCom->execute_single_command(newCommand, false);
  }
...
void qm_plistview::dropEvent(QDropEvent *e) {
  ...
  if (e->source() == this) // drag from inside playlist
  {
    ...
    if (topLevelItemCount() == 0 || !b_mpd_connected || tobeMoved.empty() ||
        tobeMoved.size() == topLevelItemCount()) {
      this->setFocus();
      line->hide();
      dropBeforeIndex = -1;
      e->ignore();
  ...
  } else if (e->source() != this && e->source() != nullptr) // drag from library
  {
  ...
  } else if (e->source() == nullptr) // drag from outside the app
  {
    printf("External items dropped on playlist\n");

    if (b_mpd_connected && e->mimeData()->hasUrls() &&
        config->mpd_socket_conn) {
      QList<QUrl> urlList = e->mimeData()->urls();
      QList<QUrl> checkedlist;
      int count = 0;
      int put_it_here = dropBeforeIndex;
...
bool qm_plistview::file_check(const QString &url) {
  ...
  if (url.endsWith(".mp3", Qt::CaseInsensitive) ||
    ...
    QString msg = "File added : " + fname + "\n";
    printf("%s", msg.toUtf8().constData());
    return true;
  } else {
    QString msg = "File skipped : " + fname + "\n";
    printf("%s", msg.toUtf8().constData());
    return false;
  }
}
